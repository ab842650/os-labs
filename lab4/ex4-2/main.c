...
void uart_init() {
    ...
    *(volatile unsigned char*)UART_IER |= 0x1; // Enable RX interrupt
    *(volatile unsigned char*)UART_MCR |= 0x1 << 3; // Enable UART interrupt
...
void plic_init() {
    ...
    *(volatile unsigned int*)PLIC_ENABLE(boot_cpu_hartid) |= (1 << UART_IRQ);
    *(volatile unsigned int*)PLIC_THRESHOLD(boot_cpu_hartid) = 0;
